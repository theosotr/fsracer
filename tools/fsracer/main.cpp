...
process_args(gengetopt_args_info &args_info,
             processor::TraceProcessor &trace_proc)
{
  ...
  if (args_info.fault_detector_given) {
    ...
    if (args_info.ignore_files_conf_given) {
      args.ignore_files_conf = args_info.ignore_files_conf_arg;
    }
...
main(int argc, char **argv)
{
  ...
  trace_generator::StreamTraceGenerator trace_gen (args_info.trace_file_arg);
  cmdline_parser_free(&args_info);
  trace_gen.Start();
  ...
  while (trace_gen.HasNext()) {
    ...
    if (trace_node) {
      trace_proc.AnalyzeTrace(trace_node);
      delete trace_node;
    }
  }
  if (trace_gen.HasFailed()) {
